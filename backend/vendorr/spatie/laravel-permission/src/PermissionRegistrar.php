...
class PermissionRegistrar
{
    ...
    {
        $this->permissionClass = config('permissions.models.permissions');
        $this->roleClass = config('permissions.models.role');
        $this->teamResolver = new (config('permissions.team_resolver', DefaultTeamResolver::class));
    ...
    {
        $this->cacheExpirationTime = config('permissions.cache.expiration_time') ?: \DateInterval::createFromDateString('24 hours');

        $this->teams = config('permissions.teams', false);
        $this->teamsKey = config('permissions.column_names.team_foreign_key', 'team_id');

        $this->cacheKey = config('permissions.cache.key');

        $this->pivotRole = config('permissions.column_names.role_pivot_key') ?: 'role_id';
        $this->pivotPermission = config('permissions.column_names.permission_pivot_key') ?: 'permission_id';
    ...
    {
        // the 'default' fallback here is from the permissions.php config file,
    ...
    /**
     * Register the permissions check method on the gate.
    ...
    public function setPermissionClass($permissionClass)
    {
        ...
        config()->set('permissions.models.permissions', $permissionClass);
    ...
    public function setRoleClass($roleClass)
    {
        ...
        config()->set('permissions.models.role', $roleClass);
    ...
    {
        $this->except = config('permissions.cache.column_names_except', ['created_at', 'updated_at', 'deleted_at']);
