...
trait HasPermissions
{
    ...
    public function getWildcardClass()
    {
        ...
        if (config('permissions.enable_wildcard_permission')) {
            $this->wildcardClass = config('permissions.wildcard_permission', WildcardPermission::class);
    ...
    public function permissions(): BelongsToMany
    {
        $relation = $this->morphToMany(
            ...
            'model',
            config('permissions.table_names.model_has_permissions'),
            config('permissions.column_names.model_morph_key'),
    ...
    public function scopePermission(Builder $query, $permissions, $without = false): Builder
    {
        ...
        return $query->where(fn (Builder $query) => $query
            ...
            ->whereIn(config('permissions.table_names.permissions').".$permissionKey", \array_column($permissions, $permissionKey))
            )
            ->when(count($rolesWithPermissions), fn ($whenQuery) => $whenQuery
                ...
                ->whereIn(config('permissions.table_names.roles').".$roleKey", \array_column($rolesWithPermissions, $roleKey))
                )
            )
        );
    }
     ...
     * whether indirectly by role or by direct permissions.
     *
     * @param  string|int|array|Permission|Collection|\BackedEnum  $permissions
     */
    ...
    /**
     * Determine if the model may perform the given permissions.
     *
     * @param  string|int|Permission|\BackedEnum  $permission
    ...
    /**
     * Validates a wildcard permissions against all permissions of a user.
     *
     * @param  string|int|Permission|\BackedEnum  $permission
     * @param  string|null  $guardName
     */
    ...
    /**
     * Determine if the model has, via roles, the given permissions.
     */
    ...
    /**
     * Determine if the model has the given permissions.
     *
     * @param  string|int|Permission|\BackedEnum  $permission
     *
     * @throws PermissionDoesNotExist
    ...
    /**
     * Grant the given permissions(s) to a role.
     *
     * @param  string|int|array|Permission|Collection|\BackedEnum  $permissions
     * @return $this
     */
    public function givePermissionTo(...$permissions)
    {
        ...
        if (config('permissions.events_enabled')) {
    ...
    /**
     * Revoke the given permissions(s).
     *
