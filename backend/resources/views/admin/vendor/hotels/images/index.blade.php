...
<div class="container-fluid">
    <x-page-title title="Manage Images - {{ $hotel->name }}" :breadcrumbs="[['label' => 'Vendor', 'url' => route('admin.vendor.dashboard')], ['label' => 'Hotels', 'url' => route('admin.vendor.hotels.index')], ['label' => 'Images']]" />
    <x-alert />
                    ...
                    @if($images->count() > 0)
                        <div class="row" id="images-container">
                            ...
                            <div class="col-md-4 mb-3 image-item" data-id="{{ $image->id }}" data-sort="{{ $image->sort_order }}">
                                <div class="card">
                                    <img src="{{ $image->image_url }}" class="card-img-top" style="height: 200px; object-fit: cover;" alt="{{ $image->alt_text ?? $hotel->name }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('upload-form').addEventListener('submit', function(e) {
        ...
        fetch(`/admin/vendor/hotels/${hotelId}/images`, {
            method: 'POST',
            body: formData,
            headers: {
    ...
    document.querySelectorAll('.set-banner-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            fetch(`/admin/vendor/hotels/${hotelId}/images/${imageId}`, {
                method: 'PUT',
                headers: {
    ...
    function updateSortOrders() {
        ...
        fetch(`/admin/vendor/hotels/${hotelId}/images/reorder`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
