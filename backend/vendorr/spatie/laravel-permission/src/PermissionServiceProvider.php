...
class PermissionServiceProvider extends ServiceProvider
{
    public function boot()
    {
        ...
        $this->callAfterResolving(Gate::class, function (Gate $gate, Application $app) {
            if ($this->app['config']->get('permissions.register_permission_check_method')) {
    ...
    protected function offerPublishing(): void
    {
        ...
        $this->publishes([
            __DIR__.'/../config/permission.php' => config_path('permissions.php'),
        ], 'permissions-config');
    ...
    protected function registerOctaneListener(): void
    {
        ...
        if (! $this->app['config']->get('permissions.register_octane_reset_listener')) {
            return;
        }
        // @phpstan-ignore-next-line
        $dispatcher->listen(function (\Laravel\Octane\Contracts\OperationTerminated $event) {
    ...
    {
        $this->app->bind(PermissionContract::class, fn ($app) => $app->make($app->config['permissions.models.permissions']));
        $this->app->bind(RoleContract::class, fn ($app) => $app->make($app->config['permissions.models.role']));
    }
    ...
    protected function registerAbout(): void
    {
        ...
        AboutCommand::add('Spatie Permissions', static fn () => [
            'Features Enabled' => collect($features)
                ->filter(fn (string $feature, string $name): bool => $config->get("permissions.{$feature}"))
                ->keys()
                ->whenEmpty(fn (Collection $collection) => $collection->push('Default'))
                ->join(', '),
            'Version' => InstalledVersions::getPrettyVersion('spatie/laravel-permissions'),
        ]);
    }
}
